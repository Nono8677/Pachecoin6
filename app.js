...
return `<div class="crypto-card" onclick="viewPair('${s}')"> <div class="card-info"> <span class="pair-name">${s}</span> <span class="live-price" id="price-${s}">${state.livePrices[s] ? state.livePrices[s].toLocaleString('en-US', {minimumFractionDigits:2}) + '$' : '...'}</span> </div> <div class="verdict ${isBuy ? 'buy' : 'out'}">${isBuy ? "J'ACHÈTE" : 'HORS MARCHÉ'}</div> ${d.nocheco.bosType ?`<div class="bos-badge bos-${d.nocheco.bosType}">BOS ${d.nocheco.bosType.toUpperCase()}</div>`: ''} </div>`;
}).join(’’);
}
...
const ms = unit - (now.getTime() % unit); // CORRECTION
