...
/* ——————————————————————————————————————————————————————————————————————————
   1. CONFIGURATION SUPABASE (PROJET PACHECOIN6)
   —————————————————————————————————————————————————————————————————————————— */
...
const portfolio = {
    ...
    save() {
        ...
        } catch(e) { console.warn('Erreur sauvegarde Storage:', e); }
    },
...
/* ——————————————————————————————————————————————————————————————————————————
   3. AUTHENTIFICATION (Login / Signup / Logout)
   —————————————————————————————————————————————————————————————————————————— */
...
/* ——————————————————————————————————————————————————————————————————————————
   5. LOGIQUE DE MISE À JOUR ET RENDU
   —————————————————————————————————————————————————————————————————————————— */
...
function renderSignals() {
    ...
    container.innerHTML = CONFIG.pairs.map(s => {
        ...
        const isBuy = score >= 2;
        const priceStr = state.livePrices[s] ? state.livePrices[s].toLocaleString('en-US') + '$' : '...';
        
        return `
        <div class="crypto-card" onclick="viewPair('${s}')">
            <div class="card-info"><span class="pair-name">${s}</span><span class="live-price" id="price-${s}">${priceStr}</span></div>
            <div class="verdict ${isBuy ? 'buy' : 'out'}">${isBuy ? "J'ACHÈTE" : "HORS MARCHÉ"}</div>
            <div class="score-row">
                <span class="score-dot ${d.ut==='bull'?'on':'off'}">UTBot</span>
                <span class="score-dot ${d.st==='bull'?'on':'off'}">SuperTrend</span>
                <span class="score-dot ${d.qqe==='bull'?'on':'off'}">QQE</span>
            </div>
            ${d.nocheco.bosType ? `<div class="bos-badge ${d.nocheco.bosType}">${d.nocheco.bosType === 'bull' ? '🔺 BOS Bull' : '🔻 BOS Bear'}</div>` : ''}
        </div>`;
    }).join('');
}
...
function refreshPortfolio(pair) {
    ...
    <div class="portfolio-card">
        <div style="color:#f0b90b; font-weight:bold;">${pair} (${state.selectedTf.toUpperCase()})</div>
        ...
        <p style="font-size:0.8rem; opacity:0.6;">Statut : ${data.position ? 'EN POSITION' : 'LIQUIDE'}</p>
    </div>`;
}
...
function updateCountdown() {
    const now = new Date();
    let unit = state.selectedTf === '1h' ? 3600000 : (state.selectedTf === '4h' ? 14400000 : 86400000);
    let ms = unit - (now % unit);
